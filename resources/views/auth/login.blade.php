...
<body>
    ...
    <main>
        ...
        <div class="self-center">
            <p class="text-sm font-semibold text-[#57C271]">Welcome back</p>
            <h1 class="mt-3 text-4xl font-bold text-[#0F2447]">Sign in to manage your reservations</h1>
            <p class="mt-4 leading-7 text-[#64748B]">Check upcoming bookings, upload payment proof, and keep your play schedule organized.</p>
        </div>
        <section class="card">
            <form class="grid gap-5" method="POST" action="{{ route('login') }}">
                @csrf
                <label class="grid gap-2 text-sm font-semibold">Email
                    <input class="rounded-lg border border-[#E5E7EB] px-4 py-3 font-normal" type="email" name="email" value="{{ old('email') }}" required autofocus>
                </label>
                ...
                <div class="flex items-center justify-between gap-4 text-sm">
                    <label class="flex items-center gap-2"><input type="checkbox" name="remember" class="rounded border-[#E5E7EB]"> Remember me</label>
                    <a class="font-semibold text-[#0F2447]" href="{{ route('password.request') }}">Forgot password?</a>
                </div>
