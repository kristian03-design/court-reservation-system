...
class ReportController extends Controller
{
    ...
    {
        $cachedData = \Illuminate\Support\Facades\Cache::remember('admin_reports_data', 3600, function () use ($reportService) {
            $now        = Carbon::now();
            ...
            // Database-agnostic monthly bookings group query
            $driver = \Illuminate\Support\Facades\DB::connection()->getDriverName();
            if ($driver === 'pgsql') {
                $periodSql = "to_char(reservation_date, 'YYYY-MM')";
            } else {
                $periodSql = "strftime('%Y-%m', reservation_date)";
            }

            $monthlyReservations = Reservation::selectRaw("{$periodSql} as period, count(*) as total")
                ->groupBy('period')
                ->orderBy('period')
                ->get();
        ...
        return view('admin.reports.admin-reports', $cachedData);
    }
}
